...
/*1.Які існують типи даних у Javascript? ---------У JavaScript є 8 основних типів. 1) Number 2 ) Boolean 3) string 4) Symbol 
5 ) Undefined 6 )null
7) Object 8 )BigInt
...
Цей тип зазвичай використовується для зберігання значень так/ні: true означає “так, вірно”,
 а false означає “ні, не вірно”.
...
8)У JavaScript, тип “number” не може містити числа більші  9007199254740991, 
або менші за -9007199254740991 для від’ємних чисел. Це технічне обмеження, спричинене їхньою внутрішньою реалізацією.

Для більшості потреб цього достатньо, але бувають випадки, коли нам потрібні дійсно великі числа, 
наприклад, для криптографії або мікроксекундних часових міток (timestamps).
...
Оператор == (рівність): Виконує нестроге порівняння значень, допускаючи перетворення типів. 
Наприклад, при порівнянні рядків і чисел, рядки будуть перетворюватись у числа перед порівнянням. 
Оператор === (строга рівність): Виконує строге порівняння значень, не допускаючи перетворення типів. 
Обидва операнди повинні мати однаковий тип і однакове значення, щоб повернути true. 
Отже, основна різниця між == і === полягає у тому, що === перевіряє не тільки значення, але й типи даних, 
тоді як == дозволяє перетворення типів для виконання порівняння.
...
Також зустрічав трохи інші формалювання В JavaScript, оператор - це символ або символьна комбінація, 
яка виконує певну операцію над одним або більше операндами
 (змінними, значеннями або виразами) і повертає результат. 
 Оператори використовуються для виконання різних дій, таких як присвоєння значень, арифметичні операції, порівняння, 
 логічні операції тощо.*/
